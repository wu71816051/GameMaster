...
### 1.3 已实现功能 ✅ (2026-01-25 更新)

- **骰子系统** ✅ - 完整的掷骰机制和表达式解析
  - DiceParser 完整表达式解析
  - 高级功能: kh/kl/dh/dl, r/rr, ! (爆骰)
  - 命令: .r, .rd, .rh (注意: .ra 已重新定义为 CoC7 成长骰)
- **角色卡系统** ✅ - 完整的角色数据模型和管理
...
- **技能检定系统** ✅ - 基础检定系统 (Generic 规则)
  ...
  - 支持手动和自动技能值
  - 支持修正值
- **规则引擎** ✅ - 多规则系统架构 + GenericAdapter + CoC7Adapter
  - RuleSystemAdapter 抽象基类
  - RuleSystemRegistry 单例注册表
  - GenericAdapter (通用规则)
  - **CoC7Adapter** (克苏鲁 7版规则) ⭐ 新增
- **会话导出** ✅ - 多格式导出功能
  - ConversationExportService
  - 支持 text, markdown, JSON 格式
- **规则专属命令系统** 🟡 - 设计阶段 → 实现阶段 ⭐ 新增
  - 命令前缀机制: `.coc7.`, `.dnd.`
  - 会话级命令注册和软注销
  - CoC7 专属命令: `.san`, `.ra` (成长骰), `.对抗`, `.奖励骰/惩罚骰`
  - 详见: [rule-specific-commands-design.md](./rule-specific-commands-design.md)

### 1.4 待实现功能 ❌

- **高级规则适配器** ⏳ - D&D 3R 特定规则（待实现）
  - CoC7 ✅ 已完成（5级成功等级系统）
  - D&D 3R ❌ THAC0/AC系统（91个规则文档已准备）
- **自动修正值** ❌ - 属性加值、熟练度计算
- **Excel导入导出** ❌ - 角色数据批量管理
- **战斗系统** ❌ - 无先攻、回合、伤害计算
- **叙事工具** ❌ - 无 NPC/场景管理

**注意**：D&D 5e 适配器已移除，未来将实现 D&D 3R 适配器以符合设计文档要求。
...
│     用户交互接口                                 │
│  - 通用命令 (.r, .check, .char)                 │
│  - 规则专属命令 (.coc7.san, .dnd.先攻) ⭐       │
│  - 命令前缀系统                                  │
├─────────────────────────────────────────────────┤
│     命令注册管理层 ⭐ (2026-01-25 新增)          │
...
│  - GenericAdapter (通用)                        │
│  - CoC7Adapter (克苏鲁 7版) ✅                  │
│  - DnD3RAdapter (龙与地下城 3.5版) 🟡           │
│  - 规则专属命令封装 (CoC7Commands, DnDCommands) │
├─────────────────────────────────────────────────┤
│     数据层 (Models/Database)                    │
│     数据持久化                                    │
└─────────────────────────────────────────────────┘
...
4. **命令注册模式 ⭐ (2026-01-25 新增)**
   - 会话级命令动态注册
   - 软注销机制（运行时验证）
   - 三层别名策略（完整前缀、短前缀、无前缀）
   - 规则隔离避免命令冲突

5. **策略模式 ⭐ (2026-01-25 新增)**
   - 不同规则系统的专属命令封装
   - 每个规则适配器包含独立的命令集
   - 统一的命令验证接口
...
| rule_system | string | 规则系统（'coc7', 'dnd3r', 'generic'） |
...
扩展 metadata：
- `rule_system` (string?) - 'coc7', 'dnd3r', 'generic'
...
- **CoC 7e**：克苏鲁的呼唤第 7 版
- **D&D 3R**：龙与地下城第 3.5 修订版（未来）
- **可扩展**：易于添加新规则系统

#### 规则系统特性对比

| 特性 | Generic | CoC 7e | D&D 3R |
|------|---------|-------|--------|
| 骰子类型 | d100 | d100 | d20 |
...
### 阶段 4: 规则专属命令系统（第 6-7 周）⭐ 2026-01-25 更新

**目标**：规则系统专属命令和动态命令注册

1. **命令注册架构**（2-3 天）
   - 扩展 RuleSystemAdapter 基类
   - 实现 CommandRegistryService
   - 命令前缀系统
   - 软注销机制
...
### 阶段 5: D&D3R 规则适配器（第 8 周+）

**目标**：D&D 3.5 规则完整支持

1. **DnD3RAdapter 核心规则**（3-4 天）
   - d20 + 修正值 vs DC 检定系统
   - THAC0/AC 系统
   - 属性调整值计算
   - 熟练度加值
...
2. 实现核心方法：
   ...
   - 默认技能（运动、隐匿等）
   - 结果格式化（D&D 风格）
   - **getCommandPrefix()** - 返回 `.dnd.` ⭐ 新增
   - **registerCommands()** - 注册专属命令 ⭐ 新增
   - **getRuleCommands()** - 返回命令列表 ⭐ 新增
3. 在规则注册表中注册
4. 在配置中启用
...
### 8.3 添加规则专属命令 ⭐ 2026-01-25 新增

**示例**：为 CoC7 添加 `.战技` 命令

**步骤**：
1. 在 `CoC7Commands` 类中添加新命令定义
2. 实现命令处理函数 `handleManeuver()`
3. 遵循标准验证模式：
   - 调用 `validateInConversationCommand()` 验证
   - 执行命令逻辑
   - 返回格式化结果
4. 命令自动通过 `registerCommands()` 注册
5. 自动获得三层别名（完整前缀、短前缀、无前缀）
...
### 决策 5: 规则专属命令使用前缀机制 ⭐ 2026-01-25 新增

**选择**：命令前缀 + 软注销 + 三层别名

**核心决策**：
- **命令格式**：`.<规则>.<命令>`（如 `.coc7.san`, `.dnd.先攻`）
- **三层别名**：完整前缀、短前缀（`.c7.san`）、无前缀（`.san`，仅对应规则会话）
- **软注销**：命令永久存在，通过运行时验证禁用
- **命令分类**：
  - 通用命令（`.r`, `.rh`, `.check`, `.char`）- 无前缀
  - 规则专属命令（`.coc7.san`, `.dnd.先攻`）- 加规则前缀
- **会话级注册**：创建/加入会话时自动注册对应规则命令

**理由**：
- **避免冲突**：不同规则的命令不会冲突
- **用户友好**：
  - 在对应规则会话中可省略前缀（`.san` 在 CoC7 会话中可用）
  - 跨规则会话使用完整前缀（`.coc7.san` 在任何会话中明确指定）
- **易于识别**：通过前缀清楚知道命令所属规则
- **动态切换**：会话切换时自动验证，无需重启
- **可扩展**：添加新规则命令只需在适配器中定义

**权衡**：
- ✅ 规则隔离、避免冲突
- ✅ 动态管理、运行时验证
- ✅ 用户友好、易于使用
- ⚠️ 需要实现软注销机制（不能真正注销命令）
- ⚠️ 需要维护会话规则状态
...
| 规则系统 | 特定的 TRPG 规则集（如 CoC 7e, D&D 3R） |
...
#### 9. 高级规则适配器 🟡 50%
- **CoC7Adapter** ✅ **已完成 (2026-01-24)**
  - ✅ 5级成功等级系统 (大成功、极难成功、困难成功、成功、失败、大失败)
  - ✅ DB 加值计算 (基于 STR+SIZ 查表)
  - ✅ 技能熟练度系统 (闪避自动计算 DEX/2)
  - ✅ 预定义技能列表 (40+ 技能)
  - ✅ 技能名称映射（中文↔英文，50+ 映射）
  - 📁 文件: [src/rule/coc7/coc7-adapter.ts](../src/rule/coc7/coc7-adapter.ts)
- **DnD3RAdapter** ❌ (待实现，D&D 5e 已移除)
  - THAC0 和 AC 系统
  - 属性调整值计算
  - 熟练度加值
  - 91 个规则文档已准备：[docs/rule/dnd3r/](../docs/rule/dnd3r/)
- **依赖**: ✅ 基础架构已完成，可直接实现
- **状态**: D&D 5e 适配器已完全移除（2026-01-25），为 DnD3R 实现让路
...
│       ├── base/                # 抽象基类
│       ├── generic/             # 通用规则
│       ├── coc7/                # CoC7 规则 (已实现 2026-01-24)
│       │   ├── coc7-adapter.ts  # CoC7 适配器 (392行)
│       │   ├── coc7-defaults.ts # 默认技能和常量 (170行)
│       │   └── index.ts         # 模块导出
│       └── registry.ts          # 注册表
├── tests/                       # 测试 (11个测试文件)
└── docs/                        # 文档 (包含 91个 D&D 3R 规则文档)
```

---

### 整体完成度: 90% (更新于 2026-01-25)
...
6. **规则引擎架构** (100%)
   ...
   - 注册表
   - GenericAdapter
   - ✅ **CoC7Adapter** (新增 2026-01-24)

7. **技能检定系统** (100%) ⬆️
   - 基础检定完整
   - Generic 规则实现
   - ✅ **CoC7 规则实现** (已实现 2026-01-24)
   - 命令集成
...
1. **规则专属命令系统** (50%) 🟡
   - ✅ 设计完成（2026-01-25）
   - 🟡 架构实现中
   - CoC7 命令：`.san`, `.ra`, `.对抗`, `.奖励骰/惩罚骰`
   - D&D3R 命令：`.先攻`, `.豁免`, `.法术`（接口预留）
   - 详见：[rule-specific-commands-design.md](./rule-specific-commands-design.md)

2. **高级规则适配器** (50%) 🟡
   - ✅ CoC7 (已完成 2026-01-24)
   - ❌ D&D 3R (待实现，D&D 5e 已移除)
...
4. **文档齐全** - 完整的技术文档和用户指南（包含 91 个 D&D 3R 规则文档）
5. **可扩展性强** - ✅ CoC7 已实现，D&D 3R 预留接口，D&D 5e 已移除
6. **生产就绪** - 当前功能已可投入实际 CoC7 游戏使用

---

### 下一步开发方向

#### 优先级 1: 实现 D&D 3R 规则适配器
- DnD3RAdapter - THAC0/AC 系统
- 基于已准备的 91 个规则文档
- **预计时间**: 5-7 天
...
**重要变更**：
- 2026-01-25：完全移除 D&D 5e 适配器，为 D&D 3R 实现让路
- 2026-01-24：完成 CoC7 规则适配器

**关键发现**:
1. 所有基础设施（会话、消息、权限、骰子、角色）已完全实现
2. 技能检定系统的所有依赖都已满足，可以立即开始实现
3. **规则专属命令系统**设计完成，正在实现中 ⭐ 2026-01-25
4. 只需要实现技能检定系统和规则引擎，就能成为完整的 TRPG 游戏系统
...
## 📋 规则专属命令系统快速参考 ⭐ 2026-01-25 新增

### 命令分类

| 类型 | 命令示例 | 是否加前缀 | 说明 |
|------|---------|-----------|------|
| **通用命令** | `.r`, `.rh`, `.check`, `.char` | ❌ 无前缀 | 所有规则系统通用 |
| **CoC7 专属** | `.san`, `.ra`, `.对抗` | ✅ `.coc7.` 或 `.c7.` | 仅 CoC7 规则 |
| **D&D3R 专属** | `.先攻`, `.豁免`, `.法术` | ✅ `.dnd.` | 仅 D&D3R 规则 |
...
# 无前缀（仅在对应规则会话中可用）
# 在 CoC7 会话中：.san 50 0/1d6 ✅
# 在 D&D 会话中：.san 50 0/1d6 ❌（建议使用 .coc7.san）
```
...
- [CoC7 实现状态](./coc7-implementation-status.md) - CoC7 命令实现状态
